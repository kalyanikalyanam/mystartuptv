...
class BlogList extends React.Component {
  ...
  componentDidMount() {
    ...
    ref.once("value", (snapshot) => {
      ...
      this.setState(
        { BlogList: data, countPage: data.length, loading: false },
        () => {
          console.log(this.state.BlogList, "BlogList");
        }
      );
  ...
  deleteItem = (id) => {
    swal({
      ...
      if (willDelete) {
        console.log(id);
        var playersRef = firebase.database().ref(`/Blog/${id}`);
        playersRef.remove();
                                ...
                                <div className="row">
                                  <div className="col-md-12">
                                    <div className="col-md-6">
                                      ...
                                      >
                                        <Link
                                          to={`/Admin/EditBlog/${items.blogId}`}
                                        >
                                          <i
